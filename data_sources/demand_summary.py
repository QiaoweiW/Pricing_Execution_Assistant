...
# ── Plan-history tracker sync (Refresh-from-Fabric) ──────────────────────────
#
# The Demand Plan Comparison reads ``qry_mgmt_plan_history_tracker.csv`` —
# the *cycle-over-cycle* history.  It is just ``qry_mgmt_plan_full.csv``
# (same 7 columns) stamped with a ``Cycle`` label and appended over time.
# Rather than maintain a Fabric notebook cell for that append, the
# Demand Summary "Refresh from Fabric" button calls
# :func:`sync_history_tracker_with_full`, which snapshots the current full
# plan into the tracker as the next ``Cn`` cycle — but only when that
# snapshot isn't already the tracker's latest cycle (idempotent).

# Canonical tracker schema + column order for the written CSV.
_TRACKER_COLUMNS: list[str] = [
    "Start of Month", "Item", "Item Description", "Party Site Number",
    "Demand Plan Pounds", "Forecast Type", "Business Unit", "Cycle",
]
# "Already appended?" sample-check tuning.  We compare a random sample of
# the full snapshot's rows against the tracker's latest cycle; if nearly
# all are present AND the row counts line up, the snapshot is already in
# the tracker and we skip the append.
_SYNC_SAMPLE_SIZE: int = 500
_SYNC_MATCH_THRESHOLD: float = 0.98


def _content_keys(df: pd.DataFrame) -> pd.Series:
    """Per-row content key ``date|item|site|ftype|lbs`` for cross-file match.

    Collapses the two files' differing text formats to one comparable
    form so ``qry_mgmt_plan_full`` (``2026-06-01`` / ``1944.0``) and the
    tracker (``6/1/2026`` / ``1944``) key identically:

    * Start of Month  -> ISO calendar date (``YYYY-MM-DD``)
    * Item            -> :func:`_normalise_item_key` (trailing ``.0`` dropped)
    * Demand Plan Pounds -> numeric rounded to 2 dp
    """
    d = (pd.to_datetime(df["Start of Month"], errors="coerce")
           .dt.strftime("%Y-%m-%d"))
    d = d.where(d.notna(), "").astype("string")
    item = df["Item"].map(_normalise_item_key).astype("string")
    site = df["Party Site Number"].astype("string").str.strip().fillna("")
    ftype = df["Forecast Type"].astype("string").str.strip().fillna("")
    lbs = (pd.to_numeric(df["Demand Plan Pounds"], errors="coerce")
             .round(2).astype("string").fillna(""))
    return d.str.cat([item, site, ftype, lbs], sep="|", na_rep="")


def _parse_cycle_number(label: object) -> Optional[int]:
    """Extract the integer from a cycle label (``"C4"`` / ``"c04"`` -> 4)."""
    m = re.match(r"\s*[Cc]?\s*0*(\d+)\s*$", str(label))
    return int(m.group(1)) if m else None


def _next_cycle_label(existing_cycles: list[str]) -> str:
    """Return the next ``Cn`` label (max existing number + 1; ``C1`` if none)."""
    nums = [n for n in (_parse_cycle_number(c) for c in existing_cycles)
            if n is not None]
    if nums:
        return f"C{max(nums) + 1}"
    # No parseable 'Cn' labels yet — number by how many distinct cycles exist.
    return f"C{len(set(existing_cycles)) + 1}"


def _format_full_as_tracker_rows(full_df: pd.DataFrame, cycle_label: str) -> pd.DataFrame:
    """Shape ``qry_mgmt_plan_full`` rows to the tracker's schema + text style.

    Matches the file's existing look so appended rows are indistinguishable
    from prior ones: Start of Month as ``M/D/YYYY`` (no zero-pad), Demand
    Plan Pounds with a trailing ``.0`` stripped, plus the new ``Cycle``.
    """
    dt = pd.to_datetime(full_df["Start of Month"], errors="coerce")
    som = dt.map(lambda x: f"{x.month}/{x.day}/{x.year}" if pd.notna(x) else "")
    lbs = (full_df["Demand Plan Pounds"].astype("string")
                 .str.replace(r"\.0$", "", regex=True))
    out = pd.DataFrame({
        "Start of Month":     som.to_numpy(),
        "Item":               full_df["Item"].map(_normalise_item_key).to_numpy(),
        "Item Description":   full_df["Item Description"].astype("string").to_numpy(),
        "Party Site Number":  full_df["Party Site Number"].astype("string").str.strip().to_numpy(),
        "Demand Plan Pounds": lbs.to_numpy(),
        "Forecast Type":      full_df["Forecast Type"].astype("string").str.strip().to_numpy(),
        "Business Unit":      full_df["Business Unit"].astype("string").str.strip().to_numpy(),
        "Cycle":              cycle_label,
    })
    return out[_TRACKER_COLUMNS]


def sync_history_tracker_with_full() -> dict:
    """Append ``qry_mgmt_plan_full.csv`` to the plan-history tracker as a new
    ``Cn`` cycle, unless its content already is the tracker's latest cycle.

    Wired to the Demand Summary "Refresh from Fabric" button.  Reads both
    files straight from Fabric (bypassing the Streamlit cache), runs a cheap
    sample comparison of the current full snapshot against the tracker's
    latest cycle, and — only when they differ — stamps the snapshot with the
    next cycle label and appends it.  Existing tracker rows are preserved
    verbatim (read/written as text).

    Returns a small result dict for the UI to render::

        {action, cycle, rows_added, prior_cycles, history_rows, message}

    where ``action`` is one of ``"appended"`` / ``"created"`` / ``"skipped"``.
    Raises :class:`DemandSummaryError` on any read/write failure.
    """
    str_kwargs = {"dtype": str, "keep_default_na": False}

    # 1. Current full snapshot — the candidate new cycle.
    try:
        full_df, _ = read_csv(
            _SECRETS_SECTION, _MGMT_PLAN_FULL_BLOB_PATH,
            read_csv_kwargs=str_kwargs)
    except LakehouseIOError as exc:
        raise DemandSummaryError(
            f"Could not read 'Files/{_MGMT_PLAN_FULL_BLOB_PATH}': {exc}"
        ) from exc
    if full_df is None or full_df.empty:
        # Never let an empty/failed read append a blank cycle.
        return {"action": "skipped", "cycle": None, "rows_added": 0,
                "prior_cycles": [], "history_rows": 0,
                "message": ("qry_mgmt_plan_full.csv is empty — no cycle "
                            "appended to the plan-history tracker.")}

    missing = [c for c in _TRACKER_COLUMNS
               if c != "Cycle" and c not in full_df.columns]
    if missing:
        raise DemandSummaryError(
            f"qry_mgmt_plan_full.csv is missing required column(s): {missing}")

    # 2. Existing tracker — string-preserving so prior rows stay byte-stable.
    try:
        hist_df, _hist_etag = read_csv(
            _SECRETS_SECTION, _MGMT_PLAN_HISTORY_TRACKER_BLOB_PATH,
            read_csv_kwargs=str_kwargs)
    except LakehouseIOError as exc:
        raise DemandSummaryError(
            f"Could not read 'Files/{_MGMT_PLAN_HISTORY_TRACKER_BLOB_PATH}': "
            f"{exc}") from exc
    if hist_df is None:
        hist_df = pd.DataFrame(columns=_TRACKER_COLUMNS)

    existing_cycles: list[str] = []
    if "Cycle" in hist_df.columns and not hist_df.empty:
        existing_cycles = sorted(
            {c for c in hist_df["Cycle"].astype(str).str.strip() if c})

    # 3. Sample check — is the full snapshot already the latest cycle?
    if existing_cycles:
        latest_cycle = max(
            existing_cycles, key=lambda c: (_parse_cycle_number(c) or -1, c))
        latest_rows = hist_df[
            hist_df["Cycle"].astype(str).str.strip() == latest_cycle]
        latest_keys = set(_content_keys(latest_rows))
        full_keys = _content_keys(full_df)

        sample = full_keys
        if len(sample) > _SYNC_SAMPLE_SIZE:
            sample = sample.sample(_SYNC_SAMPLE_SIZE, random_state=0)
        present = float(sample.isin(latest_keys).mean()) if len(sample) else 0.0
        count_ratio = len(full_df) / max(len(latest_rows), 1)

        if present >= _SYNC_MATCH_THRESHOLD and 0.98 <= count_ratio <= 1.02:
            return {"action": "skipped", "cycle": latest_cycle,
                    "rows_added": 0, "prior_cycles": existing_cycles,
                    "history_rows": len(hist_df),
                    "message": (
                        "qry_mgmt_plan_full.csv already matches the latest "
                        f"tracker cycle {latest_cycle} ({present:.0%} of "
                        "sampled rows present) — no new cycle added.")}

    # 4. Build the new cycle's rows in the tracker's text conventions.
    new_label = _next_cycle_label(existing_cycles)
    new_rows = _format_full_as_tracker_rows(full_df, new_label)

    # 5. Concatenate (existing rows untouched) and write back.
    if not hist_df.empty:
        for col in _TRACKER_COLUMNS:
            if col not in hist_df.columns:
                hist_df[col] = ""
        hist_df = hist_df[_TRACKER_COLUMNS]
    combined = pd.concat([hist_df, new_rows], ignore_index=True)[_TRACKER_COLUMNS]
    ...
    try:
        ...
        raise DemandSummaryError(
            "Could not update "
            f"'Files/{_MGMT_PLAN_HISTORY_TRACKER_BLOB_PATH}': {exc}") from exc

    action = "created" if not existing_cycles else "appended"
    return {"action": action, "cycle": new_label, "rows_added": len(new_rows),
            "prior_cycles": existing_cycles, "history_rows": len(combined),
            "message": (
                f"Appended {len(new_rows):,} rows as cycle {new_label} to the "
                "plan-history tracker.")}
